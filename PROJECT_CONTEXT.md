...
- `ABILITIES_CALC` → ⚠️ **non la usa nessuno.** Lista di 20 nomi inglesi che nessun
  modulo importa: il pallino ● sulle abilità che incidono lo calcola
  `abilityIncideSulDanno()` in `calcolatori-core.js`, leggendo il blocco `effect`.
  Verificato l'11/08/2026 cercando in tutto il progetto. Candidata alla rimozione
