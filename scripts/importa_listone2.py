#!/usr/bin/env python
"""Porta i due Excel di fantacalcio.it nel listone della Fantacalcio 2. Rieseguibile.

    python scripts/importa_listone2.py FILE FILE --dry-run
    python scripts/importa_listone2.py FILE FILE
    python scripts/importa_listone2.py FILE FILE --calendario   # anche football-data
...
⚠️ La logica non è qui: sta in `fanta2.py`, perché la stessa cosa la fa il form della
pagina. Questo file è il rivestimento a riga di comando che stampa il rapporto.

⚠️ Tocca **solo** le tabelle `fanta2_*`: il listone della prima sezione
(`fanta_players`) resta com'è, di proposito.
"""
import argparse
import os
import sys
