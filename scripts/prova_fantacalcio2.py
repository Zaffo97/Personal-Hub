...
def prove(dove):
    ...
    esito("⚠️ e non ci si segna più niente: né pulsanti, né la route che salvava",
          ...
          and c.post("/fantacalcio2/chi-gioca/segna",
    ...
    elenco = c.get("/fantacalcio2/").data.decode("utf-8", "replace")
    ...
    esito("dentro la lega c'è il pulsante delle probabili",
          f"/fantacalcio2/lega/{lid}/chi-gioca" in
          c.get(f"/fantacalcio2/lega/{lid}").data.decode("utf-8", "replace"))
