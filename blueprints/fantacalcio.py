"""La sezione Fantacalcio: leghe, rose, formazioni, con fonti che si possono usare.

§4.6 del backlog. Nata il 24/09/2026 come «Fantacalcio 2», accanto alla sezione di
prima che leggeva le pagine di fantacalcio.it; il 25/09/2026 Davide ha tenuto questa,
la vecchia è stata tolta e questa ne ha preso il nome, le route e le tabelle. Qui:
...
Le regole delle leghe (`REGOLE`, i valori ufficiali, `_prezzo()` e `_numeri()`)
fino al 25/09/2026 si importavano dalla sezione vecchia: spenta quella, sono qui.
"""
import os
...
def _contesto_giornata(db):
    ...
    calendario_c_e = bool(db.execute(
        "SELECT 1 FROM fanta_calendario LIMIT 1").fetchone())
...
def _quanti(allerta):
    """Le cose da guardare: quelle di `quanti_guai()` più quello che **manca**."""
...
def lega_elimina(lid):
    ...
    accesi, quindi le figlie si tolgono a mano — **tutte e due** (la sezione vecchia
    toglieva la rosa e non la formazione).
    """
    db = get_db()
    cond, par = solo_mie()
    mia = f"league_id IN (SELECT id FROM fanta_leagues WHERE id=? AND {cond})"
    db.execute(f"DELETE FROM fanta_roster WHERE {mia}", (lid,) + tuple(par))
    db.execute(f"DELETE FROM fanta_formazione WHERE {mia}", (lid,) + tuple(par))
...
def lega(lid):
    ...
    return render_template(
        "fanta_lega.html", lega=riga, rosa=rosa, per_ruolo=per_ruolo,
...
def consiglio_applica(lid):
    ...
    if scelto is None:
        ...
        flash(f"Il modulo {modulo or '—'} non è fra quelli consigliabili per questa "
              "lega", "error")
        return redirect(url_for("fantacalcio.formazione", lid=lid, _anchor="consiglio"))
...
def rosa_rimuovi(lid, rid):
    ...
    cur = db.execute(f"DELETE FROM fanta_roster WHERE id=? AND league_id=? AND {mia}",
                     (rid, lid) + tuple(par))
