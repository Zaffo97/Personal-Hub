...
| ⬜ | **Lo sweep dà un falso errore su `?.[`** | Fuori scope, trovato il 14/09/2026. `moderno()` in `scripts/sweep_pagine.py` trasforma `?.` in `.`, quindi `TC[mvType]?.[dt]` in `calcolatori-danno.js` diventa `TC[mvType].[dt]` ed `esprima` lo rifiuta. Oggi non morde perché lo sweep legge solo gli script inline, non i `.js` esterni. Diventa un problema il giorno in cui glieli si fa leggere |
