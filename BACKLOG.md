...
2. I residui Pokémon sono chiusi (26/09): abilità di M-C e loro sorelle, Kingambit,
   potenza di Unghiedure e Tecnico, flag. Restano le code trovate lungo la strada (§3)
...
| | Baco | Stato |
|---|---|---|
| ⬜ | **Le «-ate» e Forza Bruta moltiplicano ancora il danno finale** | Trovato il 26/09/2026 spostando Unghiedure e Tecnico. Nei giochi anche Pellecielo & co. (×1.2, 4915/4096) e Forza Bruta (×1.3, 5325/4096) sono modificatori della **potenza**; il motore le tiene in `abilityDmgMult`. Stesso difetto delle due appena spostate (1-2 PS sul roll alto, senza errore), stessa cura: entrare in `modAbilitaPotenza`. Non fatto perché non chiesto. ⚠️ Forza Bruta nel motore vale su **ogni** mossa, e nel gioco solo su quelle con un effetto secondario: prima di spostarla va deciso anche questo |
| ⬜ | **Due flag che le liste non confermano, e Antiproiettile senza effetto** | Dal 26/09/2026. **Water Pulse** ha `bullet`: né Bulbapedia né Serebii (le mosse di Antiproiettile) glielo danno, quindi le due fonti ci sono e lo toglie `integra_flag_mosse.py --da-elenco bullet --togli`. **Rain Dance** ha `dance`, che la lista di Bulbapedia non ha (seconda fonte non letta). Oggi non cambiano nessun numero: il motore non legge né `bullet` né `dance`, e **Antiproiettile** e **Sincrodanza** hanno `effect: none` |
