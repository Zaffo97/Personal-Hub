...
| ⚠️ **La potenza si calcola in un passo solo, abilità e oggetto insieme** | Dal 26/09/2026, in `calcolatori-danno.js`. Unghiedure, Tecnico e `flag_boost` (Affilama, Ferropugno, Ferromascella, Megalancio) finiscono in `modAbilitaPotenza`, gli oggetti di potenza in `modOggPotenza`, e `bpEff = floor(bp × tutti e due)`. Arrotondarli uno dopo l'altro dà un numero diverso **senza errore**: Pugnofuoco con Ferropugno e Guantone fa 99 (×1.32, Bulbapedia) e in due passi 98. Un'abilità nuova che tocca la potenza va lì, non in `abilityDmgMult`, che è il danno finale: la differenza è 1-2 PS sul roll alto, misurata (Scizor Pugnoscarica con Tecnico 64-76 sul danno, 63-75 sulla potenza). ⚠️ E nelle prove il totale degli SP è **66**, come in Champions: 32+32+32 sul difensore non esiste, il campo ne tiene 2 |
| ⚠️ **I flag che il motore legge hanno ognuno il suo scrittore, e una mossa nuova nasce senza** | Dal 26/09/2026. `slicing`, `punch`, `bite`, `pulse`, `bullet`, `dance` vengono dalle liste di Bulbapedia (`integra_flag_mosse.py --da-elenco <flag>`, che dal 26/09 le rispecchia in tutti e due i versi); `sheer_force` da `flag_forza_bruta.py` (Bulbapedia ∩ Showdown). Una mossa aggiunta al catalogo — l'import dal pannello, una versione nuova di Champions — **non li ha** finché non si rilanciano: Affilama, Ferropugno, Forza Bruta e Antiproiettile ci passano sopra **senza errore**. Dopo ogni mossa nuova: `integra_flag_mosse.py --da-elenco <flag> --dry-run` per i sei, e `flag_forza_bruta.py --dry-run` |
| ⚠️ **Dopo una modifica al Python, l'hub acceso va riavviato** | Dal 26/09/2026. Senza `HUB_DEBUG=1` il server di sviluppo **non ricarica il codice Python, ma rilegge i template**: un template nuovo che chiama una funzione nuova (qui `link_crack`) trova il processo col codice vecchio, e la pagina risponde 500 (`UndefinedError`). Il 26/09 l'hub sulla 5000 era un processo acceso alle 16:15 dal pannello di un'altra chat: i tre avvii delle 17:47-17:48 nel log non sono serviti, perché la porta la teneva ancora lui. Il log lo dice (`/admin/log`, categoria errore); per vedere chi tiene la porta: `netstat -ano` e il PID |
...
2. I residui Pokémon sono chiusi (26/09), code comprese: abilità di M-C e sorelle,
   Kingambit, potenza delle abilità, Forza Bruta, Antiproiettile, flag, Ondascudo
