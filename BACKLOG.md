...
| ⬜ | **23 flag sbagliati che vengono da `moves_ma.json`** | Trovati il 23/09/2026 integrando i flag, e **non corretti**: vanno **tolti**, e lo script approvato aggiunge soltanto. Il file storico dei dati di MA ha messo `contact` su mosse che non lo fanno — **19 vecchie**, dove il dump dice di no (Absorb, Beak Blast, Beat Up, Bone Club, **Bulk Up** che è di stato, Glacial Lance, Grass Pledge, Icicle Crash, Metal Burst, Mud Slap, Pin Missile, Rock Tomb, Sacred Fire, Seed Bomb, Sky Attack, Snarl, Spirit Shackle, **Stone Edge**, Twineedle) e **3 di Gen 9**, dove Bulbapedia dice `touches=no` (Aqua Cutter, Gigaton Hammer, Mountain Gale) — più `punch` su **Storm Throw**, che né il dump né Bulbapedia confermano. Effetto: Unghiedure dà ×1.3 e Lanugine ×0.5 dove non dovrebbero, **senza errore**. Le note di Champions (§5.2) non toccano il contatto di nessuna di queste. `integra_flag_mosse.py` ristampa i 3 di Gen 9 a ogni giro |
